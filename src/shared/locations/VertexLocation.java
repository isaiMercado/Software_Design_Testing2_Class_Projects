...
public class VertexLocation
{
	...
	public List<HexLocation> getAdjacentHexLocations() {
		List<HexLocation> result = new ArrayList<>();
		VertexLocation normVertexLoc = getNormalizedLocation();
		HexLocation normHexLoc = normVertexLoc.getHexLoc();
		HexLocation loc;

		result.add(normHexLoc);
		result.add(new HexLocation(normHexLoc.getX(), normHexLoc.getY() - 1));

		if (normVertexLoc.getDir() == VertexDirection.NorthEast) {
			loc = new HexLocation(normHexLoc.getX() + 1, normHexLoc.getY() - 1);
		}
		else {
			// In this case normVertexLoc.getDir() == VertexDirection.NorthWest
			loc = new HexLocation(normHexLoc.getX() - 1, normHexLoc.getY());
		}
		result.add(loc);
		return result;
	}
